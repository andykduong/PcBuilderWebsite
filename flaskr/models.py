...
build_cpu = db.Table('build_cpu',
                     db.Column('build_id', db.Integer, db.ForeignKey('pcbuild.id'), primary_key=True),
                     db.Column('cpu_id', db.Integer, db.ForeignKey('cpu.id'), primary_key=True)
                     )
build_cpucooler = db.Table('build_cpucooler',
                     db.Column('build_id', db.Integer, db.ForeignKey('pcbuild.id'), primary_key=True),
                     db.Column('cpucooler_id', db.Integer, db.ForeignKey('cpucooler.id'), primary_key=True)
                     )
build_mobo = db.Table('build_mobo',
                     db.Column('build_id', db.Integer, db.ForeignKey('pcbuild.id'), primary_key=True),
                     db.Column('mobo_id', db.Integer, db.ForeignKey('mobo.id'), primary_key=True)
                     )
build_gpu = db.Table('build_gpu',
                     db.Column('build_id', db.Integer, db.ForeignKey('pcbuild.id'), primary_key=True),
                     db.Column('gpu_id', db.Integer, db.ForeignKey('gpu.id'), primary_key=True)
                     )
build_ram = db.Table('build_ram',
                     db.Column('build_id', db.Integer, db.ForeignKey('pcbuild.id'), primary_key=True),
                     db.Column('ram_id', db.Integer, db.ForeignKey('ram.id'), primary_key=True)
                     )
build_drive = db.Table('build_drive',
                     db.Column('build_id', db.Integer, db.ForeignKey('pcbuild.id'), primary_key=True),
                     db.Column('drive_id', db.Integer, db.ForeignKey('drive.id'), primary_key=True)
                     )
build_psu = db.Table('build_psu',
                     db.Column('build_id', db.Integer, db.ForeignKey('pcbuild.id'), primary_key=True),
                     db.Column('psu_id', db.Integer, db.ForeignKey('psu.id'), primary_key=True)
                     )
...
build_fans = db.Table('build_fans',
                     db.Column('build_id', db.Integer, db.ForeignKey('pcbuild.id'), primary_key=True),
                     db.Column('fans_id', db.Integer, db.ForeignKey('fans.id'), primary_key=True)
                     )
...
class PCBuild(db.Model):
    ...
    cpu = db.relationship('CPU', secondary=build_cpu, back_populates='build')
    cpucooler = db.relationship('CPUCOOLER', secondary=build_cpucooler, back_populates='build')
    mobo = db.relationship('MOBO', secondary=build_mobo, back_populates='build')
    gpu = db.relationship('GPU', secondary=build_gpu, back_populates='build')
    ram = db.relationship('RAM', secondary=build_ram, back_populates='build')
    drive = db.relationship('DRIVE', secondary=build_drive, back_populates='build')
    psu = db.relationship('PSU', secondary=build_psu, back_populates='build')
    case = db.relationship('CASE', secondary=build_case, back_populates='build')
    fans = db.relationship('FANS', secondary=build_fans, back_populates='build')
...
class CPU(db.Model):
    ...
    build = db.relationship('PCBuild', secondary=build_cpu, back_populates='cpu')
...
class CPUCOOLER(db.Model):
    ...
    build = db.relationship('PCBuild', secondary=build_cpucooler, back_populates='cpucooler')
...
class MOBO(db.Model):
    ...
    build = db.relationship('PCBuild', secondary=build_mobo, back_populates='mobo')
...
class GPU(db.Model):
    ...
    build = db.relationship('PCBuild', secondary=build_gpu, back_populates='gpu')
...
class RAM(db.Model):
    ...
    build = db.relationship('PCBuild', secondary=build_ram, back_populates='ram')
...
class DRIVE(db.Model):
    ...
    build = db.relationship('PCBuild', secondary=build_drive, back_populates='drive')
...
class PSU(db.Model):
    ...
    build = db.relationship('PCBuild', secondary=build_psu, back_populates='psu')
...
class CASE(db.Model):
    ...
    build = db.relationship('PCBuild', secondary=build_case, back_populates='case')
...
class FANS(db.Model):
    ...
    build = db.relationship('PCBuild', secondary=build_fans, back_populates='fans')